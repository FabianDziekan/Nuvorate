...
export class GoogleBusinessApiError extends Error {
  constructor(
    public readonly stage: "accounts" | "locations",
    public readonly status: number,
  ) {
    super(`${stage} request failed (${status})`);
  }
}
...
export async function googleLocations(accessToken: string): Promise<{ email: string; locations: GoogleLocation[] }> { const headers = { Authorization: `Bearer ${accessToken}` }; const accountsResponse = await fetch("https://mybusinessaccountmanagement.googleapis.com/v1/accounts", { headers, cache: "no-store" }); if (!accountsResponse.ok) { console.error(`[Google OAuth] accounts_fetch_failed status=${accountsResponse.status}`); throw new GoogleBusinessApiError("accounts", accountsResponse.status); } const accounts = (await accountsResponse.json()).accounts ?? []; const results: GoogleLocation[] = []; for (const account of accounts) { const response = await fetch(`https://mybusinessbusinessinformation.googleapis.com/v1/${account.name}/locations?readMask=name,title,metadata.placeId&pageSize=100`, { headers, cache: "no-store" }); if (!response.ok) { console.error(`[Google OAuth] locations_fetch_failed status=${response.status}`); if (response.status === 429 || response.status >= 500) throw new GoogleBusinessApiError("locations", response.status); continue; } const data = await response.json(); for (const location of data.locations ?? []) results.push({ accountId: account.name, accountName: account.accountName ?? account.name, locationName: location.name, locationTitle: location.title ?? "Lokalizacja Google", placeId: location.metadata?.placeId ?? null }); }
