...
export async function selectGoogleLocation(_previous: unknown, formData: FormData) {
  ...
  const business = (await requireActiveBusinessForUser(supabase, user.id, "id", "manage")).business; if (business.id !== pending.businessId) return { error: "Nie udało się zweryfikować firmy." };
...
export async function disconnectGoogleConnection() { const supabase = await createClient(); const { data: { user } } = await supabase.auth.getUser(); if (!user) redirect("/login?next=/settings"); const business = (await requireActiveBusinessForUser(supabase, user.id, "id", "manage")).business; const { error } = await createAdminClient().from("google_business_connections").delete().eq("business_id", business.id); if (error) return { error: "Nie udało się odłączyć Google." }; revalidatePath("/settings"); revalidatePath("/dashboard"); return { ok: true }; }
