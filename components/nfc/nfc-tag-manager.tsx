...
export function NfcTagManager({ tags }: { tags: NfcTagSummary[] }) {
  ...
  const activeTags = orderedTags.filter((tag) => tag.isActive).length;

  useEffect(() => {
    const openAddModal = () => setAdding(true);
    document.addEventListener("nfc:add-tag", openAddModal);
    return () => document.removeEventListener("nfc:add-tag", openAddModal);
  }, []);

  return (
    <>
      <section className="mt-5 rounded-[24px] border border-black/[0.06] bg-white p-4 shadow-card min-[769px]:mt-8 min-[769px]:p-5 sm:p-6">
        <div className="hidden min-[769px]:flex min-[769px]:flex-wrap min-[769px]:items-end min-[769px]:justify-between min-[769px]:gap-4">
        ...
        <div className="flex items-center justify-between gap-3 min-[769px]:hidden"><h2 className="text-lg font-semibold tracking-tight">Plakietki NFC</h2><span className="text-xs font-medium text-black/45">{activeTags} aktywne</span></div>
        {orderedTags.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-black/[0.1] bg-[#FAFAFC] px-5 py-8 text-center"><p className="text-sm text-black/50">Nie masz jeszcze plakietek NFC.</p><button type="button" className="button-secondary mt-4" onClick={() => setAdding(true)}>Dodaj pierwszą plakietkę</button></div> : <><div className="mt-6 hidden divide-y divide-black/[0.06] rounded-2xl border border-black/[0.06] min-[769px]:block">{orderedTags.map((tag) => <div key={tag.id} className="flex flex-wrap items-center gap-x-5 gap-y-3 px-4 py-4"><div className="min-w-[150px] flex-1"><p className="font-semibold">{tag.name}</p><div className="mt-2"><TagStatus active={tag.isActive} /></div></div><p className="text-sm text-black/55"><span className="font-semibold text-ink">{tag.scansLast30Days}</span> skanów / 30 dni</p><p className="text-sm text-black/45">{tag.scansTotal} łącznie</p><p className="text-sm text-black/45">Ostatni skan: {tag.lastScanLabel}</p><button type="button" className="button-secondary" onClick={() => setSelected(tag)}>Szczegóły</button></div>)}</div><div className="mt-4 space-y-2 min-[769px]:hidden">{orderedTags.map((tag) => <button key={tag.id} type="button" className="flex min-h-[92px] w-full items-center gap-3 rounded-2xl border border-black/[0.06] bg-[#FAFAFC] px-4 py-3 text-left transition active:scale-[0.99]" onClick={() => setSelected(tag)}><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-sm font-semibold">{tag.name}</p><TagStatus active={tag.isActive} /></div><p className="mt-2 text-xs text-black/55"><span className="font-semibold text-ink">{tag.scansLast30Days}</span> skanów · 30 dni <span className="text-black/30">·</span> {tag.scansTotal} łącznie</p><p className="mt-1 text-xs text-black/45">Ostatni skan: {tag.lastScanLabel}</p></div><span className="shrink-0 text-lg text-black/35" aria-hidden="true">›</span></button>)}</div></>}
      </section>
